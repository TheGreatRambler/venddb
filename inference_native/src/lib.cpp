...
// ONNX Runtime does not support concurrent construction of Ort::Env/Session,
// so serialize detector construction. Inference itself remains parallel since
// each detector owns its own session.
static std::mutex detector_load_mutex;
...
extern "C" VendDB_Detector venddb_load_detector(const char* config_yaml_path, int use_gpu) {
	try {
		...
		// Use thread-specific temp file to avoid conflicts between concurrent loads.
		auto thread_id          = std::hash<std::thread::id> {}(std::this_thread::get_id());
		std::string classes_tmp = std::string(config_yaml_path) + ".classes." + std::to_string(thread_id) + ".tmp";
		{
		...
		// For thread safety, serialize construction of Ort::Env/Session inside YOLODetector with a global mutex.
		std::lock_guard<std::mutex> lock(detector_load_mutex);
