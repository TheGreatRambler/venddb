#!/usr/bin/env python3
"""Train SigLIP2 1B (so400m) from scratch using image filenames as text labels."""
...
from transformers import AutoConfig, AutoModel, AutoProcessor, get_cosine_schedule_with_warmup
...
def _run_remote(ssh: str, script: Path, data_dir: Path, config_path: Path, local_output_dir: Path, port: int | None = None, pip_packages: list[str] | None = None) -> None:
...
def siglip_loss(image_embeds, text_embeds, logit_scale, logit_bias):
    """SigLIP sigmoid contrastive loss (Zhai et al., 2023)."""
    img = F.normalize(image_embeds, dim=-1)
    txt = F.normalize(text_embeds, dim=-1)
    logits = img @ txt.T * logit_scale.exp() + logit_bias
    n = logits.shape[0]
    # +1 on diagonal (matched pairs), -1 elsewhere
...
def main():
    parser = argparse.ArgumentParser(description="Train SigLIP2 1B from scratch")
    ...
    SIGLIP2_MODELS_WORKDIR = Path(cfg["siglip2_models_dir"]).resolve()
    ...
    if args.ssh:
        _run_remote(
            ...
            DATA_DIR,
            args.config.resolve(),
            SIGLIP2_MODELS_WORKDIR,
            port=args.ssh_port,
            pip_packages=["commentjson", "transformers", "accelerate"],
        )
    dir_hash = hashlib.md5(str(DATA_DIR).encode()).hexdigest()[:12]
    work_dir = SIGLIP2_MODELS_WORKDIR / f"siglip2_workdir_{dir_hash}"

    SIGLIP2_MODELS_WORKDIR.mkdir(parents=True, exist_ok=True)
    work_dir.mkdir(parents=True, exist_ok=True)

    log.info(f"Models dir : {SIGLIP2_MODELS_WORKDIR}")
    ...
    log.info(f"Fetching processor + config for {MODEL_ID} -> {SIGLIP2_MODELS_WORKDIR}")
    model_config = AutoConfig.from_pretrained(MODEL_ID, cache_dir=SIGLIP2_MODELS_WORKDIR)
    processor = AutoProcessor.from_pretrained(MODEL_ID, cache_dir=SIGLIP2_MODELS_WORKDIR)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for batch in loader:
            ...
            with torch.cuda.amp.autocast(enabled=use_amp):
                ...
                logit_bias = getattr(core, "logit_bias", torch.zeros(1, device=device))

                loss = siglip_loss(img_embeds, txt_embeds, logit_scale, logit_bias)
